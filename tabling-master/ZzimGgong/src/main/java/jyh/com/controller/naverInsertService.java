 ...
 */
@WebServlet("/naverInsertService")
public class naverInsertService extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public naverInsertService() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 등록 결과 
		int result = mem.naverSignUp(new memberDTO(email,nik));
		if(result>0) {
			int token = mem.naverTokenInsert(email,id);
			if(token > 0) {
				//성공시 세션처리 
				request.setAttribute("email",email);
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('회원등록완료!')");
				script.println("</script>");
			...
			// 이메일 동의를 하지 않은 경우. 
			request.setAttribute("error","SNS 인증 동의 확인 후 다시 한번 로그인해주세요.");
			PrintWriter script = response.getWriter();
			script.println("<script>");
